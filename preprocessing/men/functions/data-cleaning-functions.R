# NCAAM DATA CLEANING FUNCTIONS ----

extract_team_info_mbb <- function(team_node) {
  #####
...
scrape_bracket_mbb <- function(url, year) {
  #####
...
scrape_adv_stats_mbb <- function(url, year) {
  #####
  # This function takes in two inputs, a string url, and a year.
...
scrape_pace_mbb <- function(url, year) {
  #####
  # This function takes in two inputs, a string url, and a year.
...
scrape_bracket_ncaam <- function(url) {
  packages <- c("dplyr", "rvest") # grab packages
  for (pkg in packages) {
    if (!pkg %in% .packages()) {
      library(pkg, character.only = TRUE)
